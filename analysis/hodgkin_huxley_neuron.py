...
point_list = hodgkin_huxley_neuron_model.calc_estimated_points(initial_point, 0.01, 8000)
t_values = [point[0] for point in point_list]
stimuli = [stimulus(t) for t in t_values]

plt.style.use('bmh')
plt.plot(t_values, [point[1]['V'] for point in point_list], label="Voltage")
plt.plot(t_values, stimuli, label="stimulus", lw = 0.4)
plt.legend(loc="best")
